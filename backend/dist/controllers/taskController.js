...
exports.getUserTaskEarnings = exports.rejectTaskPayment = exports.approveTaskPayment = exports.getUserTaskSubmissions = exports.deleteTask = exports.updateTask = exports.getAllTasks = exports.claimReward = exports.submitProof = exports.getActiveTasks = void 0;
...
exports.submitProof = [
    ...
    async (req, res) => {
        try {
            ...
            const proofUrls = files.map(file => file.path); // Cloudinary returns secure_url in path property
            ...
            let userTask = await UserTask_1.default.findOne({ userId, taskId });
            if (!userTask) {
                userTask = new UserTask_1.default({ userId, taskId });
            }
...
const claimReward = async (req, res) => {
    try {
        ...
        const existingUserTask = await UserTask_1.default.findOne({ userId, taskId, completed: true });
        if (existingUserTask) {
            return res.status(400).json({ error: 'You have already completed this task' });
        }
        const userTask = await UserTask_1.default.findOne({ userId, taskId });
        ...
        if (userTask) {
            ...
            const newUserTask = new UserTask_1.default({ userId, taskId, completed: true });
...
exports.getUserTaskSubmissions = getUserTaskSubmissions;
const approveTaskPayment = async (req, res) => {
    try {
        const { submissionId } = req.body;
        const userTask = await UserTask_1.default.findById(submissionId).populate('userId').populate('taskId');
        if (!userTask) {
        ...
        // Update user balance
        const user = await User_1.default.findById(userTask.userId);
        if (!user) {
            return res.status(404).json({ error: 'User not found' });
        }
        ...
        await userTask.save();
        // Update task completedQuantity
        const task = await Task_1.default.findById(userTask.taskId);
        if (task) {
            task.completedQuantity += 1;
            task.completedBy = task.completedBy || [];
...
const rejectTaskPayment = async (req, res) => {
    try {
        const { submissionId } = req.body;
        const userTask = await UserTask_1.default.findById(submissionId);
        if (!userTask) {
            return res.status(404).json({ error: 'Submission not found' });
        }
        if (userTask.completed) {
...
exports.rejectTaskPayment = rejectTaskPayment;
const getUserTaskEarnings = async (req, res) => {
    try {
        const userId = req.user.id;
        const completedTasks = await UserTask_1.default.find({ userId, completed: true }).populate('taskId', 'reward');
...
exports.getUserTaskEarnings = getUserTaskEarnings;
